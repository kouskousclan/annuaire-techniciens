...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (signInError) {
      setError("Email ou mot de passe incorrect."); // Message générique pour la sécurité
      setLoading(false);
    } else {
      // Redirige vers la page principale après connexion
      router.push('/');
      router.refresh(); // Force la mise à jour de la session
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-2xl border border-gray-200">
        ...
        <p className="text-center text-gray-600 mb-6">
          Veuillez vous connecter pour accéder à l'annuaire.
        </p>
